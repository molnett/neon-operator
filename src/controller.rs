use crate::NeonStorage;
use crate::{telemetry, Error, Metrics, Result, StandardError};
...
async fn reconcile(doc: Arc<Document>, ctx: Arc<Context>) -> Result<Action> {
    ...
    .await
    .map_err(|e| Error::StandardError(StandardError::FinalizerError(Box::new(e))))
}
    ...
    async fn reconcile(&self, ctx: Arc<Context>) -> Result<Action> {
        ...
        if !self.was_hidden() && should_hide {
            ...
            recorder
                ...
                .await
                .map_err(StandardError::KubeError)?;
        }
        if name == "illegal" {
            return Err(StandardError::IllegalDocument); // error names show up in metrics
        }
